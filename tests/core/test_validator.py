...
def test_validator_sanitize_forbidden_eval():
    ...
    validator = InputValidator()
    with pytest.raises(
        SecurityViolationError, match="Yasakli ifade tespit edildi: eval"
    ):
...
def test_validator_sanitize_forbidden_exec():
    ...
    validator = InputValidator()
    with pytest.raises(
        SecurityViolationError, match="Yasakli ifade tespit edildi: exec"
    ):
...
def test_validator_sanitize_forbidden_import():
    ...
    with pytest.raises(
        ...
        match="Yasakli ifade tespit edildi: __import__"
    ):
...
def test_validator_sanitize_forbidden_os():
    ...
    validator = InputValidator()
    with pytest.raises(
        SecurityViolationError, match="Yasakli ifade tespit edildi: os."
    ):
...
def test_validator_sanitize_forbidden_subprocess():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: subprocess"
    ):
...
def test_validator_sanitize_forbidden_open():
    ...
    validator = InputValidator()
    with pytest.raises(
        SecurityViolationError, match="Yasakli ifade tespit edildi: open"
    ):
...
def test_validator_sanitize_forbidden_builtins():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: __builtins__"
    ):
...
def test_validator_sanitize_forbidden_globals():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: globals"
    ):
...
def test_validator_sanitize_forbidden_locals():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: locals"
    ):
...
def test_validator_sanitize_forbidden_compile():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: compile"
    ):
...
def test_validator_sanitize_forbidden_file():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: __file__"
    ):
...
def test_validator_sanitize_forbidden_name():
    ...
    with pytest.raises(
        SecurityViolationError,
        match="Yasakli ifade tespit edildi: __name__"
    ):
